...
def attribute(args):
    ...
    args = '--flow=/Users/dlee/data/repo/S3Forecast/data/chtc_in/dfFlowDams0.hdf \
            ...
            --locl=/Users/dlee/data/repo/S3Forecast/data/chtc_in/dfPredDamsLocl0.hdf \
            --lead=5'.split()

    # Arguments and Parameters control
    isnote = False
    try: 
        opts, args = getopt.getopt(args, '', ['flow=', 'glob=', 'locl=', 'note=', 'lead='])
    except getopt.GetoptError:
        print('{} --flow=<filename> --mon=<filename> --pred=<filename> --lead=<filename>'.format(sys.argv[0]))
        sys.exit(2)
    for o, a in opts:
        if o == '--flow':
            filnFlow = a
